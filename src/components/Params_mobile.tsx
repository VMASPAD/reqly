          ...
          <Card className="h-full">
            ...
            <CardContent className="space-y-4">
              ...
              {(request.body.type === 'text' || request.body.type === 'json') && (
                <div className="space-y-2">
                  <textarea
                    placeholder={request.body.type === 'json' ? '{"key": "value"}' : 'Text content'}
                    value={request.body.content || ''}
                    onChange={(e) => handleBodyUpdate({ content: e.currentTarget.value })}
                    className="w-full h-32 md:h-48 p-3 rounded-md border border-input bg-background text-sm resize-none"
                  />
                </div>
              )}
